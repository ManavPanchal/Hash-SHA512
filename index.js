function SHA(str) {
    ...
    function additionModulo_2(a, b) {
        var lsw, msw, lowOrder, highOrder;
        lsw = (a.lowOrder & 0xffff) + (b.lowOrder & 0xffff);
        msw = (a.lowOrder >>> 16) + (b.lowOrder >>> 16) + (lsw >>> 16);
        lowOrder = ((msw & 0xffff) << 16) | (lsw & 0xffff);
        lsw = (a.highOrder & 0xffff) + (b.highOrder & 0xffff) + (msw >>> 16);
        msw = (a.highOrder >>> 16) + (b.highOrder >>> 16) + (lsw >>> 16);
        highOrder = ((msw & 0xffff) << 16) | (lsw & 0xffff);
        return new int64(highOrder, lowOrder);
    }
    function additionModulo_4(a, b, c, d) {
        var lsw, msw, lowOrder, highOrder;
        lsw =
            (a.lowOrder & 0xffff) +
            (b.lowOrder & 0xffff) +
    ...
    function additionModulo_5(a, b, c, d, e) {
        var lsw, msw, lowOrder, highOrder;
        lsw =
            (a.lowOrder & 0xffff) +
            (b.lowOrder & 0xffff) +
    ...
    for (var i = 0; i < str.length; i += 32) {
        ...
        g = H[6];
        h = H[7];
        // looping 80 rounds over 1024block for 80 64 bit word blocks 
        for (var j = 0; j < 80; j++) {
            if (j < 16) {
            ...
            g = f;
            f = e;
            e = additionModulo_2(d, T1);
            d = c;
            c = b;
            b = a;
            a = additionModulo_2(T1, T2);
        }
        H[0] = additionModulo_2(a, H[0]);
        H[1] = additionModulo_2(b, H[1]);
        H[2] = additionModulo_2(c, H[2]);
        H[3] = additionModulo_2(d, H[3]);
        H[4] = additionModulo_2(e, H[4]);
        H[5] = additionModulo_2(f, H[5]);
        H[6] = additionModulo_2(g, H[6]);
        H[7] = additionModulo_2(h, H[7]);
    }
    var binarray = [];
...
function generateHash(){
    ...
    document.getElementById("shaOutput").value = SHA(input);
    document.getElementById("shaOutput").style.display = "block";
    return;
}